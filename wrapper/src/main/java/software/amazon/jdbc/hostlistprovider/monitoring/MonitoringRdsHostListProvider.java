...
public class MonitoringRdsHostListProvider extends RdsHostListProvider
  ...
  public static void clearCache() {
    monitors.getEntries().entrySet().forEach(entry -> {
      try {
  ...
  protected void clusterIdChanged(final String oldClusterId) {
    final ClusterTopologyMonitor existingMonitor = monitors.get(this.clusterId, MONITOR_EXPIRATION_NANO);
  ...
  public List<HostSpec> forceRefresh(final boolean writerImportant, final long timeoutMs)
    ...
    assert monitor != null;
    return monitor.forceRefresh(writerImportant, timeoutMs);
  }
